...
def quaternion_To_rotation_matrix_tf(quaternion):
    ...
    s = 1.0/tf.sqrt(tf.reduce_sum(tf.square(quaternion)))
    quaternion /= s
...
def positive_augmentation_tf(data):
    # data: [B, N, 3]

    data_num = data.get_shape()[0].value

    ## point cloud shrinkage
    shrinkage_factor = tf.random_uniform(shape=[data_num, 1, 1], minval=0.3, maxval=0.6, dtype=tf.float32)
    data_shrinkage = data * shrinkage_factor

    ## point cloud rotation
    angles = tf.random_uniform(shape=(data_num, 4), dtype=tf.float32)
    angles = tf.nn.l2_normalize(angles, axis=-1)
    rot_mats = batch_quaternion2mat_tf(angles)

    data_augmentation = tf.matmul(data_shrinkage, rot_mats)
